...
export default async function MessagesPage({searchParams}:{searchParams:Promise<{with?:string}>}){
 ...
 <div className="grid gap-5 md:grid-cols-[210px_1fr]"><nav aria-label="Conversations" className="space-y-1">{people.map(p=><Link key={p.id} href={`/messages?with=${p.id}`} aria-current={selected?.id===p.id?"page":undefined} className={`block rounded-md p-3 text-sm ${selected?.id===p.id?"bg-accent text-accent-foreground":"border"}`}>{p.full_name}{unread?.some(m=>m.sender_id===p.id)&&<span className="ml-2 text-xs font-semibold">New</span>}</Link>)}</nav>
